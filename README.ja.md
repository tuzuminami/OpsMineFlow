...
通常利用では、[GitHub Releases](https://github.com/tuzuminami/OpsMineFlow/releases) から署名済みの `OpsMineFlow_*.dmg` をダウンロードし、**OpsMineFlow.app** をApplicationsへドラッグして開く。ローカルruntimeの起動・終了はアプリが管理するため、Terminal・Python・Node.js・ブラウザURL・APIキー・LLMは不要。

ソースの `./scripts/run_local.sh` は開発者専用の、意図的に安全性を下げたブラウザ補助である。`OPSMINEFLOW_INSECURE_BROWSER_DEV_API=1` を明示しない限り起動しない。使い捨てのテストデータだけに使い、顧客・参加者のデータには使用しない。
