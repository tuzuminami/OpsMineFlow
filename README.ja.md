...
**ホーム > データ収集を始める** を押すと、現在対応している方法が表示される。

1. **CSVまたはJSONファイル**: すでに出力・作成済みのイベントログを取り込む。
2. **ActivityWatch localhost**: 収集範囲と参加者の同意を明示的に確認した場合だけ取り込む。
3. **Macの自動記録**: 現在の製品には含まれない。

WebUIを開いただけではバックグラウンド記録は始まらない。キー入力、スクリーンショット、画面録画、マイク、カメラ、パスワード、入力本文は取得しない。**データ収集を始める** は対応済みの取り込み方法を案内するボタンで、隠れた録画開始ボタンではない。
